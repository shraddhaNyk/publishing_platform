...
function ApplicationForReview() {
  ...
  const handleReviewDecision = (decision) => {
    ...
    if (selectedPaper && selectedReviewer) {
      ...
      .then(response => {
        ...
        setPapers(papers.map(paper => 
          paper.pid === selectedPaper.pid 
            ? { ...paper, [selectedReviewer === 'reviewer1' ? 'first_reviewer' : 'second_reviewer']: loggedInUserId, status: status }
            : paper
        ));
  ...
  const handleSubmitComments = () => {
    ...
    if (selectedPaper && selectedReviewer) {
      axios.post('http://localhost:3002/saveComments', {
        paperId: selectedPaper.pid,
        reviewer: selectedReviewer,
        comments: comments,
        type: decisionType
      ...
      .then(response => {
        setPapers(papers.map(paper => 
          paper.pid === selectedPaper.pid 
            ? { ...paper, [selectedReviewer === 'reviewer1' ? 'Reviewer_1_comments' : 'Reviewer_2_comments']: comments }
            : paper
        ));
      ...
      setError("Please select a paper and a reviewer before submitting comments.");
    }
  };
...
export default ApplicationForReview;
