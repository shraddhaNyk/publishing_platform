...
function Submit() {
  ...
  // State to store the selected book file
  const [bookfile, setbookfile] = useState(null);
  // State to control the visibility of the book message box
  const [showbookMessageBox, setShowbookMessageBox] = useState(false);

   // State to store the selected book file
   const [bookFilePlagiarism, setbookFilePlagiarism] = useState(null);
   // State to control the visibility of the book message box
   const [showbookPlagiarismMessageBox, setShowbookPlagiarismMessageBox] = useState(false);

  // State to store book title
  const [bookTitle, setbookTitle] = useState('');
  // State to control book title error message
  ...
  // State to store fetched technologies
 const [technologies, setTechnologies] = useState([]);
 const [industries, setIndustries] = useState([]);

 // Fetch technology names from the backend when the component mounts
 useEffect(() => {
 ...
     });
  ...
  // Function to handle file selection for book
  const handlebookFileSelect = (event) => {
    const selectedFile = event.target.files[0];
    const allowedTypes = ['application/pdf']; // Only allow PDF files
    ...
    if (selectedFile && allowedTypes.includes(selectedFile.type)) {
      ...
      // Show the message box for book
      setShowbookMessageBox(true);
      // Optionally, you can clear the selected file here
      event.target.value = null;
    }
  };
   // Function to handle file selection for book
   const handlebookFilePlagiarismSelect = (event) => {
    const selectedFile = event.target.files[0];
    const allowedTypes = ['application/pdf']; // Only allow PDF files

    if (selectedFile && allowedTypes.includes(selectedFile.type)) {
      // Handle the selected file (e.g., store it in state)
      setbookFilePlagiarism(selectedFile);
    } else {
      // Show the message box for book
      setShowbookPlagiarismMessageBox(true);
      // Optionally, you can clear the selected file here
      event.target.value = null;
    }
  };
...
const handleUpload = () => {
    ...
    if (bookTitle.trim() === '') {
      setbookTitleError('Please enter book title.');
      return;
    }
  ...
  formData.append('bookFilePlagiarism',bookFilePlagiarism);
  ...
  axios.post('http://localhost:3002/submitBook', formData, {
    headers: {
  ...
  return (
    <div style={{ width: '90%', height: '85vh', margin: 'auto', background: 'white', borderRadius: '10px', padding: '0px' }}>
      <div>
        <div style={{ fontFamily: 'Cambria'}}>
          {/* Section Title */}
          <h5 style={{ fontWeight: 'normal', fontSize: '30px', padding: '1%', marginTop: '0.5%', marginLeft: '13%' }}> Submit book</h5>
          ...
          <div style={{ width: '100%', height: 'auto', margin: 'auto', background: '#d1d1d1', paddingTop: '2%', paddingBottom:'2%', fontFamily: 'Cambria', display: 'flex' }}>
            ...
            {currentPage === 1 && (
             <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: '10px'  }}>
                <label htmlFor="bookTitle" style={{fontSize: '125%', marginRight: '2%', width: '150px', textAlign: 'left' }}>Book Title:</label>
                <input
                ...
                <span style={{ color: 'red', marginLeft: '10px' }}>{bookTitleError}</span>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
                ...
                {/* Error message for technology */}
                <span style={{ color: 'red', marginLeft: '10px' }}>{technologyError}</span>
              </div>
              
              {/* Technology Dropdown */}
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
                <label htmlFor="Industries" style={{ fontSize: '125%', marginRight: '2%', width: '150px', textAlign: 'left' }}>Technologies:</label>
                <select
                  id="Industries"
                  className=" mr-2 block w-72 border border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 p-2"
            ...
            {currentPage === 2 && (
              <div><h2 className='font-normal mr-32'>Please enter your details of the book in the following formats which other can use to cite your book</h2>
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
                <label htmlFor="mlaCitation" style={{ fontSize: '125%', marginRight: '2%', textAlign: 'left' }}>MLA Citation:</label>
                <textarea
                  className="mt-2 ml-8 block w-96 border border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 p-2"
                  type="text"
                  id="mlaCitation"
                  value={mlaCitation}
              ...
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
                <label htmlFor="apaCitation" style={{ fontSize: '125%', marginRight: '2%', textAlign: 'left' }}>APA Citation:</label>
                <textarea
                  className="mt-2 ml-9 block w-96 border border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 p-2"
                  type="text"
                  id="apaCitation"
                  value={apaCitation}
              ...
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
                <label htmlFor="chicagoCitation" style={{ fontSize: '125%', marginRight: '2%', textAlign: 'left' }}>Chicago Citation:</label>
                <textarea
                  className="mt-2 ml-2 block w-96 border border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 p-2"
                  type="text"
                  id="chicagoCitation"
              ...
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
                <label htmlFor="harvardCitation" style={{ fontSize: '125%', marginRight: '2%', textAlign: 'left' }}>Harvard Citation:</label>
                <textarea
                  className="mt-2 ml-2 block w-96 border border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 p-2"
                  type="text"
                  id="harvardCitation"
              ...
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
                <label htmlFor="vancouverCitation" style={{ fontSize: '125%', marginRight: '1%', textAlign: 'left' }}>Vancouver Citation:</label>
                <textarea
                  className="mt-2 ml-1 block w-96 border border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 p-2"
                  type="text"
              ...
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
                <label htmlFor="Reference_mla_citation" style={{ fontSize: '125%', marginRight: '2%', textAlign: 'left' }}>MLA Citation:</label>
                <textarea
                  className="mt-2 ml-8 block w-96 border border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 p-2"
                  type="text"
              ...
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
                <label htmlFor="Reference_apa_citation" style={{ fontSize: '125%', marginRight: '2%', textAlign: 'left' }}>APA Citation:</label>
                <textarea
                  className="mt-2 ml-9 block w-96 border border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 p-2"
                  type="text"
              ...
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
                <label htmlFor="Reference_chicago_citation" style={{ fontSize: '125%', marginRight: '2%', textAlign: 'left' }}>Chicago Citation:</label>
                <textarea
                  className="mt-2 ml-2 block w-96 border border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 p-2"
                  type="text"
              ...
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
                <label htmlFor="Reference_harvard_citation" style={{ fontSize: '125%', marginRight: '2%', textAlign: 'left' }}>Harvard Citation:</label>
                <textarea
                  className="mt-2 ml-2 block w-96 border border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 p-2"
                  type="text"
              ...
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
                <label htmlFor="Reference_vancouver_citation" style={{ fontSize: '125%', marginRight: '1%', textAlign: 'left' }}>Vancouver Citation:</label>
                <textarea
                  className="mt-2 ml-1 block w-96 border border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 p-2"
                  type="text"
        ...
        {currentPage === 4 && (
          <div>
              ...
              <div style={{ marginTop: '3.5%' }}>
                ...
                {/* book Section */}
                <div style={{ marginTop: '3%', display: 'flex', flexDirection: 'column' }}>
                  <label htmlFor="bookFile" style={{ fontSize: '125%', marginBottom: '1%' }}>Book</label>
                  {/* File input for selecting the book */}
                  <input
                    type="file"
                    id="bookfile"
                    name="bookfile"
                  ...
                  style={{ fontSize: '125%', marginLeft: '10px', color: 'blue', cursor: 'pointer', marginBottom: '1%' }}>Browse</label>
                  {/* Display the selected book file name */}
                  {bookfile && <span style={{ marginLeft: '10px' }}>{bookfile.name}</span>}
                </div>

                    {/*Plagiarisum book Section */}
                <div style={{ marginTop: '3%', display: 'flex', flexDirection: 'column' }}>
                  <label htmlFor="bookFilePlagiarism" style={{ fontSize: '125%', marginBottom: '1%' }}>  Book Plagiarisum Report</label>
                  {/* File input for selecting the book */}
                  <input
                    type="file"
                    id="bookFilePlagiarism"
                    name="bookFilePlagiarism"
                    style={{ display: 'none' }}
                    onChange={handlebookFilePlagiarismSelect}
                    accept=".pdf" // Only accept PDF files
                  />

                  {/* Browse button for selecting file */}
                  <label htmlFor="bookFilePlagiarism" 
                  style={{ fontSize: '125%', marginLeft: '10px', color: 'blue', cursor: 'pointer', marginBottom: '1%' }}>Browse</label>
                  {/* Display the selected book file name */}
                  {bookFilePlagiarism && <span style={{ marginLeft: '10px' }}>{bookFilePlagiarism.name}</span>}
                </div>
              </div>
              </div>
      ...
      {showbookMessageBox && (
        <div className="message-box">
          <p>Please select a PDF file for the book.</p>
          <button onClick={() => setShowbookMessageBox(false)}>OK</button>
        </div>
      )}
       {/*Plagiarisum book Message Box */}
       {showbookPlagiarismMessageBox && (
        <div className="message-box">
          <p>Please select a PDF file for the Plagiarisum book.</p>
          <button onClick={() => setShowbookPlagiarismMessageBox(false)}>OK</button>
        </div>
      )}
